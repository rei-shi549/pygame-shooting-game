...
pygame.display.set_caption("pygame-シューティングゲーム")
    ...
    while running:
        ...
        if boss:
            ...
            if boss.flash_timer > 0:
                if boss.flash_timer % 4 < 2:
                    boss.color = (255, 255, 255)
                else:
                    boss.color = (0, 0, 0)
                boss.flash_timer -= 1
            else: 
                boss.color = boss.base_color
        ...
        # ボスの攻撃（ランダムに弾を発射）
        if boss and random.random() < 0.02:
